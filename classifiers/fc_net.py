...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dout2, dW2, db2 = affine_backward(dsoftmax, cache3)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in np.arange(1, self.num_layers, 1):
          ...
          # Optional batchnorm
          if self.use_batchnorm:
            temp_out, temp_cache = batchnorm_forward(temp_out, self.params["gamma" + str(i)], self.params["beta" + str(i)], self.bn_params[i - 1])
            batch_cache[i] = temp_cache
          # ReLu
          temp_out, temp_cache = relu_forward(temp_out)
          relu_cache[i] = temp_cache
          # Optional Dropout
          if self.use_dropout:
            temp_out, temp_cache = dropout_forward(temp_out, self.dropout_param)
            dropout_cache[i] = temp_cache
          last_input = temp_out
        i = self.num_layers
        ...
        #dW_number of layers - 1, db_number of layers - 1; ... dW1, db1
        for i in range(self.num_layers - 1, 0, -1):
          if self.use_dropout:
            dlast = dropout_backward(dlast, dropout_cache[i])
          dlast = relu_backward(dlast, relu_cache[i]) 
          if self.use_batchnorm:
            dlast, dgamma, dbeta = batchnorm_backward(dlast, batch_cache[i])
